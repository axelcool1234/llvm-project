...
bool ProximityMemoryLoadsPass::isProximity(const uint64_t ElemSizeA,
                                           Value *PtrA,
                                           const uint64_t ElemSizeB,
                                           Value *PtrB, const uint64_t N) {
...
uint64_t ProximityMemoryLoadsPass::getPtrConst(const uint64_t ElemSize,
                                               Value *Ptr,
...
uint64_t ProximityMemoryLoadsPass::calcPtrWithOffset(const uint64_t ElemSize,
                                                     GetElementPtrInst *GEP,
                                                     const bool SameBaseAddr) {
  uint64_t BaseAddr = 0;
  if (auto *BasePtr = dyn_cast<ConstantInt>(GEP->getPointerOperand())) {
    BaseAddr = BasePtr->getZExtValue();
  } else if (!SameBaseAddr)
    return 0;
